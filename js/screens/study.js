...
let activeUnit = null;
function unitFromParam(p) { const m = /^u(\d+)$/.exec(p || ''); return m ? Number(m[1]) : null; }
function pool() { const cs = store.getCards(); return activeUnit ? cs.filter(c => c.unitId === activeUnit) : cs; }

export const study = {
  id: 'study',
  title: () => 'Study',
  mount(view, ctx, param) {
    activeUnit = unitFromParam(param);
    const cards = pool();
...
function setMeta(ctx, dueCount) {
  ...
  const u = activeUnit ? unitById(activeUnit) : null;
  const unitChip = u ? `<span class="unit-chip">Unit ${u.id}</span>` : '';
  const streakHtml = streak > 0 ? `<span class="streak">${ic.flame}${streak}</span>` : '';
  ctx.setMeta(`${unitChip}${streakHtml}<span><b>${dueCount}</b> due today</span>`);
}
...
function renderLanding(view, ctx, count) {
  ...
  view.querySelector('[data-act="start"]').addEventListener('click', () => {
    startSession(view, ctx, dueCards(pool()));
  });
}
...
function renderCardStage(view, ctx, state) {
  ...
  stage.querySelectorAll('[data-rate]').forEach(btn => btn.addEventListener('click', () => {
    ...
    state.reviewed++;
    state.i++;
    setMeta(ctx, dueCards(pool()).length);
    renderCardStage(view, ctx, state);
  }));
}
...
function renderComplete(view, ctx, state) {
  view.innerHTML = '';
  const stillDue = dueCards(pool()).length;
  view.appendChild(elFrom(`
    <div class="center-state">
