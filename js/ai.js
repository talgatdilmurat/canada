...
// Strict-JSON discipline: prefill "{", strip fences, parse, validate, retry once.
...
// ---------- Photo page -> candidate vocabulary (Sonnet, vision) — spec §5.5a ----------
export async function extractCandidates(base64, mediaType) {
  const system = cached('You extract study vocabulary from a page of the official "Discover Canada" citizenship guide, for a Turkish native speaker learning English. Return only JSON.');
  const content = [
    { type: 'image', source: { type: 'base64', media_type: mediaType, data: base64 } },
    { type: 'text', text:
`Extract the vocabulary worth learning from this page — meaningful words and key terms, NOT trivial/function words ("the", "and") or words a basic learner already knows. For each: a very simple English meaning and the Turkish meaning.
Return ONLY: {"candidates":[{"term":"...","english":"...","turkish":"..."}]}
Output JSON only.` },
  ];
  const j = await callJSON({ model: MODELS.sonnet, system, content, max_tokens: 1000 });
  return Array.isArray(j.candidates)
    ? j.candidates.filter(c => c && c.term).map(c => ({ term: String(c.term).trim(), english: String(c.english || '').trim(), turkish: String(c.turkish || '').trim() }))
    : [];
}

// ---------- Ask: grounded Turkish Q&A about one chapter (Sonnet) — spec §5.5b ----------
