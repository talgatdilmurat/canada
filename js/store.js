...
const K = { cards: 'lexikon-ca:cards', settings: 'lexikon-ca:settings', meta: 'lexikon-ca:meta' };
...
export function addCard(data) {
  ...
  const card = {
    id: uid(),
    unitId: Number.isInteger(data.unitId) ? data.unitId : null,
    term,
...
export function importDeck(json) {
  ...
  for (const raw of incoming) {
    ...
    cards.push({
      id: raw.id || uid(),
      unitId: Number.isInteger(raw.unitId) ? raw.unitId : null,
      term: raw.term,
...
// ---------- units ----------
export function cardsInUnit(unitId) { return getCards().filter(c => c.unitId === unitId); }
export function unitStats(unitId) {
  const cs = cardsInUnit(unitId);
  const learned = cs.filter(c => c.fsrs && c.fsrs.state === 2 && c.fsrs.stability >= 21).length;
  return { total: cs.length, learned };
}
